...
async function checkMetaAndShowBanner(root, seasonId) {
  ...
  if (!res || res.ok !== true) {
    // Don't spam a toast; just show a tiny banner message
    renderBanner(root, `
  ...
  // If cache has no open matches, show banner to prompt Update even if fingerprint unchanged
...
function renderMatchList(root, seasonId, openMatches) {
  ...
  const open = (openMatches||[]).slice().sort((a,b)=>{
    const ad = new Date(`${normalizeDateStr(a.date)}T${normalizeTimeStr(a.time)}:00`).getTime()||0;
    const bd = new Date(`${normalizeDateStr(b.date)}T${normalizeTimeStr(b.time)}:00`).getTime()||0;
    return ad-bd;
  });
...
if (playersRes.ok) {
  ...
  // render past from cache if exists
  renderPastArea(root, seasonId);

  // meta banner check (non-blocking)
...
async function renderMatchDetail(root, code) {
  ...
  // 1) Render instantly from cache if present
  ...
  // 2) Build UI using cached data (no waiting on players list)
  const m = data.match;
  ...
  detail.querySelector("#refreshMatchBtn").onclick = async () => {
    ...
    // Re-render detail from fresh data (no nav)
    await renderMatchDetail(root, code);
  };

  renderAvailLists();

  // 3) Load players list in background and populate dropdown
